...
const Perfil = () => {
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        <Grid item xs={12} md={esSeller ? 6 : 12}>
          <Paper elevation={3} sx={{ p: 4, borderRadius: 3, height: '100%' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                ...
                <Box sx={{ position: 'relative' }}>
                  <input accept="image/jpeg,image/png" id="input-foto" type="file"
                    style={{ display: 'none' }} onChange={handleCambiarFoto} />
                  <IconButton onClick={handleFotoClick} disabled={subiendo}
                    sx={{ p: 0, '&:hover .overlay': { opacity: 1 } }}>
                    <Avatar src={perfil?.fotoUrl}
                      sx={{ width: 90, height: 90, fontSize: '2.5rem', bgcolor: 'primary.main' }}>
            ...
            {perfil?.descripcion ? (
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" color="text.secondary">
                  {t.lang === 'en' ? 'Description' : 'Descripción'}
                </Typography>
            ...
            {/* Mis publicaciones (solo sellers) */}
            {esSeller && (
              <Button variant="contained" fullWidth startIcon={<StorefrontIcon />}
            ...
            {perfil?.role === 'USER' && !perfil?.solicitudVendedor && (
              <Box sx={{ mt: 2 }}>
                <Alert severity="info" sx={{ mb: 1.5 }}>
                  {t.lang === 'en'
                    ? 'Want to sell? Request to become a seller.'
                    : '¿Quieres vender? Solicita ser vendedor.'}
                </Alert>
                <Button variant="contained" fullWidth onClick={handleSolicitarVendedor}
                  sx={{ borderRadius: 2 }}>
                  {t.requestSeller}
                </Button>
              </Box>
                ...
                <>
                  ...
                  <Grid container spacing={2} sx={{ mb: 3 }}>
                    <Grid item xs={12} sm={6}>
                      <StatCard icon={<AccountBalanceWalletIcon />} label={t.availableBalance}
                        value={`$${Number(perfil?.saldoDisponible || 0).toFixed(2)}`}
                        color="#10b981" isDark={isDark} />
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <StatCard icon={<AttachMoneyIcon />} label={t.totalEarned}
                        value={`$${totalGanancias.toFixed(2)}`}
                        color="#6366f1" isDark={isDark} />
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <StatCard icon={<ReceiptLongIcon />} label={t.completedSales}
                        value={totalVentas}
                        color="#f59e0b" isDark={isDark} />
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <StatCard icon={<HourglassEmptyIcon />} label={t.pending}
                  ...
                  {totalVentas > 0 && (
                    <Box sx={{
                      ...
                      <Typography variant="body2" color="text.secondary">{t.avgPerSale}</Typography>
                  ...
                  {ventas.length === 0 ? (
                    <Box sx={{ textAlign: 'center', py: 4 }}>
                      ...
                      <Typography color="text.secondary">{t.noSalesYet}</Typography>
                    </Box>
                  ) : (
                        ...
                        <React.Fragment key={venta.id}>
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', py: 1.5 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                              <Avatar src={venta.producto?.urlPortada} variant="rounded"
                              ...
                              <Box>
                                ...
                                <Typography variant="caption" color="text.secondary">
                                  {venta.cliente?.nombreMostrado || venta.cliente?.username || 'Cliente'}{' · '}
                                  {venta.fechaPedido
                                    ? new Date(venta.fechaPedido).toLocaleDateString('es-MX', { day: '2-digit', month: 'short' })
                                    : ''}
                                </Typography>
                              </Box>
                            </Box>
                            <Box sx={{ textAlign: 'right' }}>
                              ...
                              <Chip label={venta.estado} color={estadoColor(venta.estado)}
      ...
      {/* ── MODAL MIS COMPRAS ── */}
      <Dialog open={comprasOpen} onClose={() => setComprasOpen(false)}
        maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 4 } }}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pb: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <ShoppingBagIcon sx={{ color: 'primary.main' }} />
            <Typography variant="h6" fontWeight="bold">{t.myPurchases}</Typography>
          </Box>
        ...
        <DialogContent sx={{ p: 0 }}>
          {loadingCompras ? (
            ...
            <Box sx={{ textAlign: 'center', py: 6 }}>
              ...
              <Typography color="text.secondary">{t.noPurchasesYet}</Typography>
            </Box>
          ) : (
            <List disablePadding>
                  ...
                  <ListItem sx={{ px: 3, py: 2 }}>
                    ...
                    <ListItemText sx={{ ml: 1.5 }}
                      ...
                      secondary={
                        <Box>
                          <Typography variant="caption" color="text.secondary">
                            {t.lang === 'en' ? 'Seller' : 'Vendedor'}: {pedido.producto?.vendedor?.nombreMostrado || 'N/A'}
                          </Typography>
                          <br />
                          <Typography variant="caption" color="text.secondary">
                            {pedido.fechaPedido
